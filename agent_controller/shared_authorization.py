...
def _valid_record(record: SharedAuthorizationRecord) -> bool:
    ...
    if record.state is AuthorizationState.PROPOSED:
        return record.provenance is None and record.execution_claim_id is None and record.controller_run_id is None
    if record.state is AuthorizationState.HUMAN_APPROVED:
        return _valid_provenance(record.provenance) and record.execution_claim_id is None and record.controller_run_id is None
    if record.state in {
        ...
        return _valid_provenance(record.provenance) and _nonempty(record.execution_claim_id) and _nonempty(record.controller_run_id)
    return record.execution_claim_id is None and record.controller_run_id is None


def _valid_snapshot(snapshot: object) -> bool:
    return isinstance(snapshot, SharedRecordSnapshot) and _nonempty(snapshot.revision) and _valid_record(snapshot.record)
...
def propose_operation(*, store: SharedAuthorizationStore, binding: OperationAuthorizationBinding) -> SharedAuthorizationResult:
    ...
    try:
        write = store.backend.create_if_absent(state_ref=store.state_ref, path=path, record=record)
    except Exception:
    ...
    if outcome == "WRITTEN":
        return SharedAuthorizationResult(AuthorizationDecision.PASS, SharedRecordSnapshot(record, write.revision))
