...
def canonicalize_context_capsule(capsule: ContextCapsule) -> bytes:
    if not isinstance(capsule, ContextCapsule):
        raise TypeError("capsule must be ContextCapsule")
    facts = [json.loads(canonicalize_capsule_fact(fact)) for fact in sorted(capsule.facts, key=lambda item: item.key)]
    payload = {
        "controller_task_id": capsule.controller_task_id,
        "facts": facts,
...
class ContextRetrievalPlanner:
    ...
    def _reusable_fact(self, *, capsule: ContextCapsule, fact: CapsuleFact) -> bool:
        if any(pointer.freshness is not EvidenceFreshness.IMMUTABLE for pointer in fact.evidence):
            return False
        if any(pointer.authority not in {EvidenceAuthority.OBJECTIVE_VERIFIED, EvidenceAuthority.CONTROLLER_MEASURED} for pointer in fact.evidence):
            return False
        try:
            return self._verified_source.is_verified_fact(bound_capsule_fact_digest(capsule=capsule, fact=fact)) is True
        except Exception:
            return False
        ...
        *,
        ...
        for request in requests:
            ...
            if request.required_for_security_gate:
                decisions.append(RetrievalDecision(request.key, RetrievalAction.FETCH_FOR_SECURITY_GATE, "SECURITY_GATE_REQUIRES_FRESH_EVIDENCE"))
                continue
            if request.key in unknowns or request.key not in facts:
                decisions.append(RetrievalDecision(request.key, RetrievalAction.FETCH_MISSING, "EVIDENCE_UNKNOWN_OR_MISSING"))
                continue
            fact = facts[request.key]
            if request.key in must_recheck or any(pointer.freshness is not EvidenceFreshness.IMMUTABLE for pointer in fact.evidence):
                decisions.append(RetrievalDecision(request.key, RetrievalAction.FETCH_MUTABLE, "MUTABLE_EVIDENCE_RECHECK_REQUIRED"))
                continue
            if self._reusable_fact(capsule=capsule, fact=fact):
                decisions.append(RetrievalDecision(request.key, RetrievalAction.REUSE_VERIFIED_IMMUTABLE, "EXACT_VERIFIED_OPERATION_BOUND_IMMUTABLE_FACT"))
                continue
            decisions.append(RetrievalDecision(request.key, RetrievalAction.FETCH_MISSING, "OPERATION_BOUND_IMMUTABLE_FACT_NOT_CONFIRMED_BY_VERIFICATION_SOURCE"))
