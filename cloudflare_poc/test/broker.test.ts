...
import {
  BrokerLedgerDO, BrokerWitnessDO, activateBroker, brokerRuntimeIdentity, brokerStatus,
  claimBroker, stageLedgerAfterRestore, stageWitnessAfterRestore, suspendBrokerForRecovery,
  type ActivationCertificate, type RecoveryAuthorization,
} from "../src/index";
...
async function sign(value: Record<string, string>, privateJwkJson: string): Promise<string> {
  const key = await crypto.subtle.importKey("jwk", JSON.parse(privateJwkJson), "Ed25519", false, ["sign"]);
...
async function activationCertificate(epoch: string, changes: Partial<ActivationCertificate> = {}) {
  ...
  if (!ledgerStatus.local_recovery_nonce || !witnessStatus.local_recovery_nonce) throw new Error("RECOVERY_NONCES_UNAVAILABLE");
...
async function recoveryAuthorization(newEpoch: string) {
  const identity = await brokerRuntimeIdentity(env);
  const current = await brokerStatus(env);
  if (current.decision !== "ACTIVE" || !current.broker_epoch || !current.activation_digest) throw new Error("RECOVERY_REQUIRES_ACTIVE_BROKER");
  const auth: RecoveryAuthorization = {
    ...
    recovery_id: `recovery-${current.broker_epoch}-to-${newEpoch}`,
    ledger_object_id: identity.ledger_object_id,
    witness_object_id: identity.witness_object_id,
    broker_authority_id: identity.broker_authority_id,
    expected_current_epoch: current.broker_epoch,
    expected_activation_digest: current.activation_digest,
...
async function snapshotControl(stub: any) {
  return runInDurableObject(stub, async (_instance, state) =>
    state.storage.sql.exec<Record<string, string | null>>("SELECT * FROM control WHERE singleton=1").one());
}
...
async function completeRecovery(oldLedger: Record<string,string|null>, oldWitness: Record<string,string|null>, recovery: Awaited<ReturnType<typeof recoveryAuthorization>>) {
  const { ledger, witness } = await stubs();
  ...
  expect((await stageLedgerAfterRestore(env,recovery.auth,recovery.signature)).decision).toBe("PASS");
  await restoreControlForTest(witness,oldWitness);
  expect((await brokerStatus(env)).decision).not.toBe("ACTIVE");
  expect((await stageWitnessAfterRestore(env,recovery.auth,recovery.signature)).decision).toBe("PASS");
}
...
describe("dual Durable Object broker authority", () => {
  ...
  it("requires exact dual signed activation", async () => {
    ...
    expect((await brokerStatus(env)).broker_authority_id).toBe(identity.broker_authority_id);
  });

  it("partial activation is fail-closed but exact retry completes idempotently", async () => {
    ...
    expect((await brokerStatus(env)).decision).not.toBe("ACTIVE");
    expect((await activateBroker(env,signed.cert,signed.signature)).decision).toBe("PASS");
    expect((await brokerStatus(env)).decision).toBe("ACTIVE");
  });
  ...
  it("concurrent identical claims have at most one logical PASS", async () => {
    ...
    const results = await Promise.all([claimBroker(env,signed.challenge,signed.signature),claimBroker(env,signed.challenge,signed.signature)]);
  ...
  it("witness-first partial claim remains consumed on ordinary retry", async () => {
    ...
    expect((await witness.reserve(digest,{activation_digest:status.activation_digest!,broker_authority_id:status.broker_authority_id!,broker_epoch:status.broker_epoch!})).decision).toBe("PASS");
    const retry = await claimBroker(env,signed.challenge,signed.signature);
    expect(retry).toEqual(expect.objectContaining({decision:"BLOCKED",reason:"PARTIAL_AUTHORIZATION_ALREADY_CONSUMED"}));
  });
  ...
  it("one-side consumed-record deletion cannot create a second PASS when counterpart remains", async () => {
    ...
    expect(first.decision).toBe("PASS");
    const { witness } = await stubs();
    await runInDurableObject(witness,async (_instance,state)=>{state.storage.sql.exec("DELETE FROM consumed WHERE authorization_digest=?",first.authorization_digest);});
  ...
  it("stored Ledger signature corruption is integrity BLOCK, not a clean replay", async () => {
    await activate("epoch-ledger-tamper");
    const signed = await signedChallenge("epoch-ledger-tamper");
    const first = await claimBroker(env,signed.challenge,signed.signature);
    expect(first.decision).toBe("PASS");
    const { ledger } = await stubs();
    await runInDurableObject(ledger,async (_instance,state)=>{state.storage.sql.exec("UPDATE attempts SET signature_b64='AAAA' WHERE authorization_digest=?",first.authorization_digest);});
    const replay = await claimBroker(env,signed.challenge,signed.signature);
    expect(replay).toEqual(expect.objectContaining({decision:"BLOCKED",reason:"STORED_AUTHORIZATION_INTEGRITY_FAILED"}));
  });

  it("one-side activation tamper suspends the front gate", async () => {
    await activate("epoch-tamper");
    const { witness } = await stubs();
    await runInDurableObject(witness,async (_instance,state)=>{state.storage.sql.exec("UPDATE control SET broker_epoch='epoch-attacker' WHERE singleton=1");});
    expect((await brokerStatus(env)).decision).not.toBe("ACTIVE");
  });

  it("claim CAS rejects stale front activation after recovery starts", async () => {
    ...
    expect((await suspendBrokerForRecovery(env,recovery.auth,recovery.signature)).decision).toBe("PASS");
    const { witness } = await stubs();
    expect((await witness.reserve(await sha(signed.challenge),{activation_digest:before.activation_digest!,broker_authority_id:before.broker_authority_id!,broker_epoch:before.broker_epoch!})).reason).toBe("ACTIVATION_CHANGED");
  });
  ...
  it("witness reservation followed by recovery makes stale ledger write fail and stay consumed", async () => {
    ...
    const digest = await sha(signed.challenge);
    const expected = {activation_digest:before.activation_digest!,broker_authority_id:before.broker_authority_id!,broker_epoch:before.broker_epoch!};
  ...
  it("sequential restore interlock never resurrects dual ACTIVE old epoch", async () => {
    ...
    await completeRecovery(oldLedger,oldWitness,recovery);
    expect((await activateBroker(env,oldActivation.cert,oldActivation.signature)).decision).toBe("BLOCKED");
  ...
  it("old recovery authorization cannot be replayed from a later activation", async () => {
    await activate("epoch-recovery-a");
    const { ledger,witness } = await stubs();
    const [oldLedger,oldWitness] = await Promise.all([snapshotControl(ledger),snapshotControl(witness)]);
    const oldRecovery = await recoveryAuthorization("epoch-recovery-b");
    await completeRecovery(oldLedger,oldWitness,oldRecovery);
    const fresh = await activationCertificate("epoch-recovery-b");
    expect((await activateBroker(env,fresh.cert,fresh.signature)).decision).toBe("PASS");
    expect((await suspendBrokerForRecovery(env,oldRecovery.auth,oldRecovery.signature)).decision).toBe("BLOCKED");
  });

  it("same-epoch recovery is rejected", async () => {
    await activate("epoch-same");
    const current = await brokerStatus(env);
    const identity = await brokerRuntimeIdentity(env);
    const auth: RecoveryAuthorization = {
      schema_version:"agent-controller-broker-recovery-poc-v1",recovery_policy_id:"recovery-policy-poc-v1",recovery_id:"same-epoch",
      ledger_object_id:identity.ledger_object_id,witness_object_id:identity.witness_object_id,broker_authority_id:identity.broker_authority_id,
      expected_current_epoch:current.broker_epoch!,expected_activation_digest:current.activation_digest!,new_broker_epoch:current.broker_epoch!,signer_key_id:"human-admin-test-ephemeral",
    };
    const signature = await sign(auth as unknown as Record<string,string>,adminPrivate());
    expect((await suspendBrokerForRecovery(env,auth,signature)).decision).toBe("BLOCKED");
  });
  ...
  it("production module exposes no external reset/delete/reclaim/PITR fetch surface", async () => {
    const ledgerMethods = Object.getOwnPropertyNames(BrokerLedgerDO.prototype).map((x)=>x.toLowerCase());
    const witnessMethods = Object.getOwnPropertyNames(BrokerWitnessDO.prototype).map((x)=>x.toLowerCase());
    for (const forbidden of ["reset","delete","reclaim","restore","bookmark","pitr"]) {
      expect(ledgerMethods).not.toContain(forbidden); expect(witnessMethods).not.toContain(forbidden);
    }
