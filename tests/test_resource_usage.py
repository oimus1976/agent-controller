...
class ResourceUsageObservationTests(unittest.TestCase):
    ...
    def test_agent_reported_is_advisory_not_measured(self):
        self.assertFalse(
            observation(source=ResourceUsageSource.AGENT_REPORTED).measured_source
        )
    ...
    def test_no_fake_total_token_helper(self):
        # Provider token categories can overlap (for example reasoning details
        # may be nested in output accounting), so raw observations deliberately
        # avoid a cross-provider arithmetic total in this contract slice.
