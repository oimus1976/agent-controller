...
class SignedApprovalTests(unittest.TestCase):
    ...
    def test_changed_signer_key_id_fails_closed_before_verification(self):
    ...
    def test_empty_signer_key_id_fails_closed(self):
        changed = dataclasses.replace(self.challenge, signer_key_id="")
        result = self.verify(challenge=changed)
        self.assertFalse(result.valid)
        self.assertIn("signer_key_id", result.reason)
    ...
    def test_supported_api_has_no_public_key_or_store_override(self):
        ...
        params = set(inspect.signature(verify_signed_approval).parameters)
